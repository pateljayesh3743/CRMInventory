...
namespace CRMInventory
{
    ...
    /// </summary>
    public partial class AddSalesInvoice : Window
    {
        ...
        public AddSalesInvoice()
        {
            ...
            SalesInvoiceDataGrid.ItemsSource = salesInvoiceCollectionViewModels;
        }
        ...
        private void BookNo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                SaleInvoiceDate.IsDropDownOpen = true;
            }
        }
        ...
        private void RefNo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                Date.Focus();
        ...
        private void SalesInvoiceDataGrid_PreviewKeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                DataGridCellInfo cell = SalesInvoiceDataGrid.SelectedCells[0];
                int rowIndex = SalesInvoiceDataGrid.Items.IndexOf(cell.Item);
                int row = rowIndex;
                List<SalesInvoiceCollectionViewModel> salesInvoiceCollectionViewModels = (List<SalesInvoiceCollectionViewModel>)SalesInvoiceDataGrid.ItemsSource;
                if (col < grid.Columns.Count - 1)
                {
                    if (!string.IsNullOrEmpty(salesInvoiceCollectionViewModels[row - 1].name))
                    {
                        ...
                        double sum = 0;
                        for (int i = 0; i < SalesInvoiceDataGrid.Items.Count - 1; i++)
                        {
                        ...
                        SalesInvoiceDataGrid.ItemsSource = salesInvoiceCollectionViewModels;
                        SalesInvoiceDataGrid.Items.Refresh();
                    }
        ...
        private void Save()
        {
            ...
            if (SerialNo.Text != null && SerialNo.Text != "")
            {
                ...
                {
                    sales_master sale_master = new sales_master
                    {
                        serial_no = Convert.ToInt32(SerialNo.Text),
                        book_no = Convert.ToInt32(BookNo.Text),
                        sales_date = Date.SelectedDate,
                        party_name_id = Convert.ToInt32(PartyName.SelectedValue),
                        supplier_invoice_no = Convert.ToInt32(RefNo.Text),
                        sales_date1 = Date.SelectedDate,
                    ...
                    db.sales_master.Add(sale_master);
                    db.SaveChanges();

                    db.sales_expenses.Add(new sales_expenses
                    {
                        sales_id = sale_master.sales_id,
                    ...
                    {
                        sales_details sale_details = new sales_details
                        {
                            sales_id = sale_master.sales_id,
                        ...
                        db.sales_details.Add(sale_details);
                        db.SaveChanges();
                    }
                    MessageBox.Show("Sales save successfully.");
                    //Clear_Form();
                }
            }
            else
        ...
        private void SGST_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                this.CalculateNetAmount();
